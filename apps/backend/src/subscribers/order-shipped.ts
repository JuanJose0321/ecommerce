...
export default async function orderShippedHandler({
  ...
  if (!order) {
    logger.warn(`[order-shipped] No se encontro una orden para el fulfillment ${event.data.id}`)
    return
  }
  ...
  await sendEmail({
    to: order.email,
    subject: `Tu orden #${displayId} va en camino`,
    react: OrderShippedEmail({ displayId }),
  })
}
